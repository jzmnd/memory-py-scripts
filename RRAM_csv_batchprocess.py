...
# Filtering
ifilter = 1e-3    # Filter all devices with max current and max current differences below this level
...
for d in files:
	...
	Ianode = array(datad["ABSIanode"])
	Janode = Ianode/area
	diff_Ianode = array(mf.numDifference(datad["ABSIanode"]))
	Vanode = array(datad["Vanode"])
	diff_resistance = abs(1/array(mf.numDiff(datad["ABSIanode"], datad["Vanode"]))) - r_series
	...
	v_switch = Vanode[argmax(abs(diff_Ianode))]
	if v_switch > vfilter:
		v_set_list.append(v_switch)

		rpHRS = where(Vanode == vr_SET)[0][0]
		rpLRS = where(Vanode == vr_SET)[0][1]

		tp = "SET"
	elif v_switch < -vfilter:
		v_reset_list.append(v_switch)

		rpHRS = where(Vanode == vr_RESET)[0][1]
		rpLRS = where(Vanode == vr_RESET)[0][0]
	...
	r_HRS = average(diff_resistance_smoothed[rpHRS-vs:rpHRS+vs+1])
	r_LRS = average(diff_resistance_smoothed[rpLRS-vs:rpLRS+vs+1])
